...
class BuildWeeklyTimeTableAction
{
    ...
    public function execute(int|string $userId, Carbon $start, Carbon $end): array
    {
        ...
        while ($currentDate->lte($end)) {
            ...
            $dayEntries = $entries->filter(fn (WorkHour $entry): bool => $entry->timestamp->isSameDay($currentDate));
        ...
        $workDays = collect($days)->filter(fn (array $day): bool => ! $day['isWeekend'])->count();
